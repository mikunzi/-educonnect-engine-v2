"""Integration tests for TrialBalance use case with SQLite adapters."""
...
def test_trial_balance_projects_from_sqlite_ledger_projection(tmp_path: Path) -> None:
    db_path = tmp_path / "trial-balance-use-case.db"
    ...
    handler = TrialBalanceHandler(ledger_projection_handler=ledger_projection_handler)
    ...
    assert result.trial_balance.total_debit().amount == Decimal("20.00")
    assert result.trial_balance.total_credit().amount == Decimal("20.00")
